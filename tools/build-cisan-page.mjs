...
const style = `${inheritedStyle?.replace('第七篇・莊子', `${lesson.index}・詞三首`)}
/* 第十二篇：三首詞分節閱讀，手機頂欄優先保留篇名、進度與返回目錄。 */
.poem-heading{margin:36px 0 4px;padding:12px 0 10px;border-top:2px solid #004936;color:#004936;font-family:"BiauKai","DFKai-SB","PMingLiU",serif;font-size:1.38rem;font-weight:600;letter-spacing:.1em}.poem-heading:first-of-type{margin-top:0}.poem-heading+.passage{border-top:1px dashed #cfc5b4}
@media(max-width:650px){.bar{align-items:center;gap:8px;padding:9px 12px}.brand{flex:1;max-width:none;gap:7px;font-size:.8rem;letter-spacing:.02em;line-height:1.2}.brand>span:last-child{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.tools{width:164px;flex:0 0 164px;flex-direction:row;flex-wrap:wrap;justify-content:flex-end;gap:5px}.score{width:100%;font-size:.6rem;text-align:right;white-space:nowrap}.reset,.return-link{padding:6px 7px;font-size:.8rem;line-height:1.2}.poem-heading{margin-top:28px;font-size:1.18rem}}
`;
...
function renderPoem(source, index) {
  const match = source.match(/^【(.+?)】([\s\S]+)$/);
  if (!match) throw new Error('詞作資料格式不正確。');
  return `    <h3 class="poem-heading">${match[1]}</h3>\n    <section class="passage"><span class="num">${index + 1}</span><p>${renderInline(match[2])}</p></section>`;
}
const termData = JSON.stringify(terms);
const html = `<!doctype html>
<html lang="zh-Hant">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>《${lesson.title}》｜原文及詞解</title><style>${style}</style></head>
<body>
  ...
  <main><article class="sheet"><header class="head"><h1>${lesson.title}</h1><p>${lesson.author}　｜　原文及詞解</p></header><div class="body"><h2>${lesson.title}</h2>
${passages.map(renderPoem).join('\n')}
  ...
  <script>
    const terms=${termData}; const letters=['A','B','C','D']; const map=Object.fromEntries(terms.map(t=>[t.id,t])); const done=new Set,first=new Set,tries=new Map;let active=null,previous=null;
    ...
    function openNext(trigger){const ids=trigger.dataset.ids.split(',');open(ids.find(id=>!done.has(id))||ids[0],trigger)} function close(){bg.classList.remove('open');bg.setAttribute('aria-hidden','true');if(previous)previous.focus();active=null}
...
fs.writeFileSync(path.join(projectRoot, 'client/public/cisan-interactive.html'), html);

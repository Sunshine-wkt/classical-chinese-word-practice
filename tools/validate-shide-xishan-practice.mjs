...
const html = fs.readFileSync(path.join(projectRoot, 'client/public/shide-xishan-interactive.html'), 'utf8');
const coverage = fs.readFileSync(path.join(projectRoot, 'source/shide-xishan-annotation-coverage.md'), 'utf8');
const excerpts = fs.readFileSync(path.join(projectRoot, 'source/shide-xishan-sentence-excerpts.md'), 'utf8');

assert.equal(terms.length, 41, '筆記 27 則注釋必須拆為 41 個題目。');
assert.match(coverage, /27 則編號注釋/);
assert.match(coverage, /41 個互動詞語題目/);
...
console.log(`《${lesson.title}》驗證通過：27 則筆記注釋拆為 ${terms.length} 題；正解、選項、覆蓋表、原文位置、完整句子節錄與返回目錄連結均符合要求。`);

...
import { lesson, passages, terms } from '../source/yuwoyusuo-question-data.mjs';

const projectRoot = path.resolve(import.meta.dirname, '..');
const htmlPath = path.join(projectRoot, 'client/public/yuwoyusuo-interactive.html');
...
for (const term of terms) {
  ...
  assert.ok(sourceText.includes(term.word) || term.id === 'gou', `${term.word} 必須可在官方原文中定位。`);
  ...
  assert.ok(term.excerpt.includes(term.word) || term.id === 'gou', `${term.word} 的節錄句必須包含目標詞。`);
  assert.match(term.excerpt, /[。！？；][」』]?$/, `${term.word} 的節錄句必須以原文標點收結。`);
  assert.ok(term.excerpt.length < 90, `${term.word} 的節錄句應避免過長。`);
  assert.ok(excerpts.includes(`| ${term.word} | ${term.excerpt} |`), `${term.word} 必須記錄於節錄文件。`);
}

console.log(`《${lesson.title}》驗證通過：18 則筆記注釋拆為 ${terms.length} 題；正解、選項、覆蓋表、原文位置、完整句子節錄與返回目錄連結均符合要求。`);

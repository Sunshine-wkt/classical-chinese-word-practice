...
import { lesson, passages, terms } from '../source/shide-xishan-question-data.mjs';
...
/* 第八篇手機頂欄：長篇名以單行、截斷優先，保留分數及兩個操作按鈕的可用觸控面積。 */
@media(max-width:650px){
  ...
  .tools{width:122px;flex:0 0 122px;flex-direction:row;flex-wrap:wrap;justify-content:flex-end;gap:5px}
  .score{width:100%;font-size:.67rem;text-align:right;white-space:nowrap}
...
function renderPassage(source, index) {
  const html = source.replace(/\{\{([^|}]+)\|([^}]+)\}\}/g, (_, rawIds, word) => {
    return `<button class="term" data-id="${rawIds.split(',')[0]}" data-ids="${rawIds}">${word}</button>`;
  });
...
fs.writeFileSync(path.join(projectRoot, 'client/public/shide-xishan-interactive.html'), html);

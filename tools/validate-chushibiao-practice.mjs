...
const html = fs.readFileSync(path.join(projectRoot, 'client/public/chushibiao-interactive.html'), 'utf8');
const coverage = fs.readFileSync(path.join(projectRoot, 'source/chushibiao-annotation-coverage.md'), 'utf8');
const excerpts = fs.readFileSync(path.join(projectRoot, 'source/chushibiao-sentence-excerpts.md'), 'utf8');

assert.equal(terms.length, 52, '教師筆記 31 則注釋必須拆為 52 個題目。');
assert.equal(new Set(terms.map((term) => term.id)).size, terms.length, '每題識別碼必須唯一。');
assert.match(coverage, /31 則編號注釋/);
assert.match(coverage, /52 個互動詞語題目/);
...
console.log(`《${lesson.title}》驗證通過：31 則教師筆記注釋拆為 ${terms.length} 題；正解、選項、覆蓋表、原文位置、完整句子節錄與返回目錄連結均符合要求。`);
